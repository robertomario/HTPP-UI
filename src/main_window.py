...
class MainWindow(wx.Frame):
    ...
    def OnNew(self, e):
        ...
        if dialogFlag == wx.ID_YES:
            ...
            self.logSettings()
            self.plotter.clear()
            self.plotter.redoLegend(variables, devices, self.num_sensors)
    ...
    def OnStart(self, e):
        ...
        if is_pressed:
            ...
            if is_test_mode:
                self.rt = RepeatedTimer(5, self.simulateSensorReadings)
            else:
                self.rt = RepeatedTimer(5, self.getAllReadings)
        else:
